package com.aggrandizer.PeerJam;
...
public class SocketHandler {
    ...
    // New Event Handler for Text Chat
    @OnEvent("sendMessage")
    public void onSendMessage(SocketIOClient client, Map<String, String> data) {
        String room = users.get(client.getSessionId().toString());
        if (room != null) {
            String message = data.get("message");
            String senderId = client.getSessionId().toString();
            // Broadcast the message to all clients in the room
            server.getRoomOperations(room).sendEvent("receiveMessage", Map.of(
                    "senderId", senderId,
                    "message", message
            ));
